...
import sch.id.smkn4palembang.databinding.GridItemOpacBinding
...
class OpacAdapter(private val context: Context) :
    ...
    inner class ViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        val binding = GridItemOpacBinding.bind(view)
        fun bind(item: Book) {
            binding.apply {
                itemTitleBookTextview.text = item.title
                itemPublisherTextview.text = item.publisher?.let {
                    context.getString(R.string.publisher, item.publisher)
                } ?: "-"
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.grid_item_opac, parent, false)
        )
    }
